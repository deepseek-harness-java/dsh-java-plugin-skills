...
**help 触发（优先级最高，先于第一问）**：用户说「help / 帮助 / 你能做什么 / 有什么能力 / 不知道做什么」时，不要直接抛二选一问题，按 `references/prompt-recipes.md`「帮助菜单」节回复：一句话能力介绍（两条路径 + 三种交付形态 + 一句话即可完成开发部署）→ 99 案例分类菜单全貌（含工程效能、角色行业、行业纵深与扩展场景板块）→ 引导话术（回复编号/名称开工，或直接描述任意其他应用）。首次接触必须让用户看到「还能选什么」，而不是猜一个再说。
